...
std::string TopicFormatParser::ExtractFieldValue(const PipelineEventPtr& event,
    ...
    switch (fieldRef.type) {
        case FieldType::CONTENT: {
            ...
            const LogEvent* logEvent = event.Get<LogEvent>();
            if (!logEvent)
                return "";
            return logEvent->HasContent(fieldRef.fieldName) ? logEvent->GetContent(fieldRef.fieldName).to_string() : "";
        }
        case FieldType::TAG: {
